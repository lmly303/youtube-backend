...
const updateAccountDetails = asyncHandler(async(req,res)=>{
    ...
    const {fullName, email} = User.findById(req.body._id)
...
const updateUserCoverImage = asyncHandler(async(req,res)=>{
    ...
    if(!coverImage.url){
        throw new apiError(401, "Error while uploading the CoverIamge file!")
    }
    ...
    const user = await User.findByIdAndUpdate(
        ...
        {
            $set:{
                coverImage: avatcoverImagear.url
            }
        },
        {new: true}
    ).select("-password")
